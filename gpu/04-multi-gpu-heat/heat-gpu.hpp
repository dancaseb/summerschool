...
struct Field<DEVICE_ONLY> : FieldInfo {
  ...
  DEVICE_CALLABLE inline double& operator()(int i, int j) {return temperature(i, j);}

  // standard (i,j) syntax for getting elements
  DEVICE_CALLABLE inline const double& operator()(int i, int j) const {return temperature(i, j);}
