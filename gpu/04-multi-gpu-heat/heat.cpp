...
void Field<HOST_ONLY>::setup(int nx_in, int ny_in, ParallelData parallel)
{ 
        nx_full = nx_in;
        ny_full = ny_in;
        ...
        temperature = Matrix<double, HOST_ONLY> (nx + 2, ny + 2);
}
...
Field<HOST_ONLY>::Field() {
  temperature = Matrix<double, HOST_ONLY>();
}
