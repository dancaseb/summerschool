...
void single_writer(const std::vector<int> &localData, const char *filename)
{
    ...
    if (rank == 0)
    {
        allData.resize(localData.size() * ntasks);
    }
    MPI_Gather(localData.data(), numElementsPerRank, MPI_INT, allData.data(), numElementsPerRank, MPI_INT, 0, MPI_COMM_WORLD);

    if (rank == 0){
        FILE *fileptr = fopen(filename, "wb");
        if (fileptr != NULL)
        {
            fwrite(allData.data(), sizeof(int), allData.size(), fileptr);
            fclose(fileptr);
        }
        else
        {
